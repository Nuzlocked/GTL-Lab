...
// Pokemon Showdown sprite URLs
const SHOWDOWN_NORMAL_BASE_URL = 'https://play.pokemonshowdown.com/sprites/gen5ani/';
const SHOWDOWN_SHINY_BASE_URL = 'https://play.pokemonshowdown.com/sprites/gen5ani-shiny/';
const TOTAL_POKEMON_GEN5 = 649;

// Configuration
const outputDir = path.resolve(__dirname, '../public/collection-sprites');
const normalDir = path.join(outputDir, 'normal');
const shinyDir = path.join(outputDir, 'shiny');
const DOWNLOAD_CONCURRENCY = 10;

// Pokemon name mappings for National Dex ID to name (Gen 1-5)
...
// List of legendary and mythical Pokemon that should be locked
const LEGENDARY_MYTHICAL_POKEMON = new Set([
...
async function downloadSprite(pokemonName, isShiny = false) {
  const baseUrl = isShiny ? SHOWDOWN_SHINY_BASE_URL : SHOWDOWN_NORMAL_BASE_URL;
  const url = `${baseUrl}${pokemonName}.gif`;
  const targetDir = isShiny ? shinyDir : normalDir;
  const filename = `${pokemonName}.gif`;
  const filepath = path.join(targetDir, filename);

  try {
    // Check if file already exists
    if (fsSync.existsSync(filepath)) {
      console.log(`${isShiny ? 'Shiny' : 'Normal'} sprite already exists: ${filename}`);
      return true;
    }
    ...
    console.log(`Downloaded ${isShiny ? 'shiny' : 'normal'} sprite: ${filename}`);
    return true;
  } catch (error) {
    console.error(`Failed to download ${isShiny ? 'shiny' : 'normal'} sprite for ${pokemonName}:`, error.message);
    return false;
  }
}

async function downloadSpriteBatch(pokemonNames, isShiny = false) {
  const semaphore = Array(DOWNLOAD_CONCURRENCY).fill(null);
  let currentIndex = 0;

  const downloadNext = async () => {
    while (currentIndex < pokemonNames.length) {
      const index = currentIndex++;
      const pokemonName = pokemonNames[index];
      await downloadSprite(pokemonName, isShiny);
    }
  };
...
async function generateCollectionData() {
  ...
  for (let id = 1; id <= TOTAL_POKEMON_GEN5; id++) {
    const pokemonName = POKEMON_NAMES_BY_ID[id];
    if (pokemonName) {
      const isLegendary = LEGENDARY_MYTHICAL_POKEMON.has(pokemonName);
      
      collectionData[pokemonName] = {
        id: id,
        name: pokemonName.charAt(0).toUpperCase() + pokemonName.slice(1).replace(/-/g, ' '),
        isLegendary: isLegendary,
        normalSprite: `collection-sprites/normal/${pokemonName}.gif`,
        shinySprite: `collection-sprites/shiny/${pokemonName}.gif`,
        generation: getGeneration(id)
      };
    }
  }

  const dataFileContent = `// Auto-generated Pokemon collection data
...
export const COLLECTION_POKEMON: Record<string, CollectionPokemon> = ${JSON.stringify(collectionData, null, 2)};

export const LEGENDARY_MYTHICAL = new Set([
  ${Array.from(LEGENDARY_MYTHICAL_POKEMON).map(name => `'${name}'`).join(', ')}
]);
`;

  const dataFilePath = path.resolve(__dirname, '../src/data/collection-data.ts');
  await fs.writeFile(dataFilePath, dataFileContent);
  console.log('Generated collection data file');
}
...
async function main() {
  console.log('Starting collection sprite download...');

  try {
    await createDirectories();
    
    const allPokemonNames = Object.values(POKEMON_NAMES_BY_ID);
    
    console.log(`Downloading normal sprites for ${allPokemonNames.length} Pokemon...`);
    await downloadSpriteBatch(allPokemonNames, false);
    
    console.log(`Downloading shiny sprites for ${allPokemonNames.length} Pokemon...`);
    await downloadSpriteBatch(allPokemonNames, true);
    
    console.log('Generating collection data...');
    await generateCollectionData();
    
    console.log('Collection sprite download completed!');
  } catch (error) {
